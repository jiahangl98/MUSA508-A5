...
to_plot %>% 
  arrange(-n) %>%
  head(50) %>% 
  ggplot(aes(x = start_station_name, -n), n, fill = time_of_day, color = weekend)) +
  scale_fill_manual(values = palette4, name="Time of Day") + 
  guides(color="none") + 
...
theme(plot.subtitle = element_text(size = 9, face = "italic"),
      plot.title = element_text(size = 12, face = "bold"), 
      axis.text.x = element_text(angle = 45, hjust = 1, size = 8), # ensure station names are displayed
      axis.ticks.x = element_line(size = 0.5),  # assuming you want to keep the x-axis ticks
      axis.text.y = element_text(size = 8), 
      axis.title = element_text(size = 10), 
      panel.background = element_blank(),
      panel.border = element_rect(colour = "grey", fill = NA, size = 0.8))
...
In this section, we will conduct further exploratory analysis of our data set. 
...
```{r calculate spatial lag, message=FALSE, warning=FALSE}
neighbors <- poly2nb((bikenew), queen = TRUE)

# Then, create a listw object from the neighbors list:
listw <- nb2listw(neighbors, style="W", zero.policy=TRUE)

length(bike.panel$Trip_Count)  # This should match the length of listw
length(listw$neighbours)  # The number of spatial units in the weights list

bike.panel$first_order_lag <- lag.listw(listw, bike.panel$Trip_Count, order=1)
bike.panel$second_order_lag <- lag.listw(listw, bike.panel$Trip_Count, order=2)
bike.panel$third_order_lag <- lag.listw(listw, bike.panel$Trip_Count, order=3)
bike.panel$fourth_order_lag <- lag.listw(listw, bike.panel$Trip_Count, order=4)

# Function to create a scatter plot with a linear fit
create_plot <- function(bike.panel, lag_column, order) {
  ggplot(data, aes_string(x = lag_column, y = 'trip_count')) +
    geom_point() +
    geom_smooth(method = 'lm', col = "blue") +
    labs(
      title = paste("Rideshare trip count as a function of spatial lags"),
      subtitle = paste("One week in December, 2018"),
      x = paste(order, "Order Contiguity"),
      y = "Trip Count"
    ) +
    theme_minimal()
}
...
plots <- list(
  create_plot(data, 'first_order_lag', 'First'),
  create_plot(data, 'second_order_lag', 'Second'),
  create_plot(data, 'third_order_lag', 'Third'),
  create_plot(data, 'fourth_order_lag', 'Fourth')
)
...
for (d in 61:91) {
  for (h in 0:23) {
    ...
    if(!all(apply(coords.test_lag, 2, is.numeric))) {
      print("Some coordinates are not numeric")
      print(coords.test_lag[!apply(coords.test_lag, 2, is.numeric)])
    }

    # Use a distance threshold to define neighbors
    distance_threshold <- 0.01 # This threshold needs to be appropriate for your data scale
    neighborList.test_lag <- dnearneigh(coords.test_lag, 0, distance_threshold)
    spatialWeights.test_lag <- nb2listw(neighborList.test_lag, style="W")
    
    # If you need a lagged variable
    spacelag <- spacelag %>% mutate(lagTrip = lag.listw(spatialWeights.test_lag, Trip_Count, zero.policy = TRUE))

    # Append the results for this iteration to the lag_result data frame
...
for (d in 61:91) {
  for (h in 0:23) {
    ...
    noise <- runif(n = nrow(coords.test_lag), min = -1e-10, max = 1e-10)
    coords.test_lag[, 1] <- coords.test_lag[, 1] + noise # add noise to longitude
    coords.test_lag[, 2] <- coords.test_lag[, 2] + noise # add noise to latitude

    # Now rerun knearneigh
